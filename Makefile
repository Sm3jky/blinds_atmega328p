...
SOURCE    = main.c serial.c protocol.c bcode.c print.c
BUILDDIR = build
SOURCEDIR = ./
...
clean:
	rm -f program.hex $(BUILDDIR)/*.o $(BUILDDIR)/*.d $(BUILDDIR)/*.elf

# file targets:
$(BUILDDIR)/main.elf: $(OBJECTS)
	$(COMPILE) -o $(BUILDDIR)/main.elf $(OBJECTS) -lm -Wl,--gc-sections

program.hex: $(BUILDDIR)/main.elf
	rm -f program.hex
	avr-objcopy -j .text -j .data -O ihex $(BUILDDIR)/main.elf program.hex
